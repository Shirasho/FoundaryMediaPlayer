...
namespace FoundaryMediaPlayer.Engine.Windows
{
    ...
    /// QuickTime graph.
    /// </summary>
    public sealed class QuickTimeGraph : GraphBase
    {
        //public QuickTimeGraph(IStore store, IntPtr hWndParent, out int hr)
        //    : base(kernel, store)
        //{
        //    hr = (int) HResult.S_OK;
            
        //    ISubPicAllocatorPresenter pQTAP;
        //    IRendererHelper renderer = kernel.Get<RendererHelper>();

        //    if (GetEngine() == EVideoRenderer. && store.Media.VideoRendererType == EVideoRendererType.DX9)
        //    {
        //        var bFullscreen = kernel.Get<RendererHelper>().IsD3DFullscreen(kernel.Get<MainWindow>());
        //        if ((hr = CreateAP9(CLSID.))
        //    }
        //}
